...
static int update_vztt_param(struct vzctl_env_handle *h)
{
	...
	if (h->env_param->tmpl->type != TM_EZ)
		return 0;
		...
		int layout, int use_ostmpl, int flags)
{
	...
	if (h->env_param->disk->root == VZCTL_PARAM_OFF ||
			ostmpl[0] == '\0')
		return 0;
	if (h->env_param->tmpl->type == TM_DC) {
		vzctl_conf_add_param(h->conf, "OSTEMPLATE", ostmpl);
		if (sscanf(ostmpl, "dockerimage://%511[^:]s", buf) == 1) {
...
static int get_template_type(const char *name)
{
	if (strncmp("dockerimage://", name, 14) == 0)
		return TM_DC;
	return TM_EZ;
}
		...
		struct vzctl_env_create_param *param)
{
	...
	h->env_param->tmpl->type = get_template_type(env->tmpl->ostmpl);
