...
int env_set_cpumask(struct vzctl_env_handle *h, struct vzctl_cpumask *cpumask)
{
	...
	ret = syscall(__NR_fairsched_cpumask, veid, sizeof(cpumask->mask), cpumask->mask);
	if (ret) {
		if (errno == ENOENT)
...
int env_set_nodemask(struct vzctl_env_handle *h, struct vzctl_nodemask *nodemask)
{
	...
	ret = syscall(__NR_fairsched_nodemask, veid, sizeof(struct vzctl_nodemask), nodemask->mask);
	if (ret) {
		if (errno == ENOENT)
