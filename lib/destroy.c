...
static int validate_ve_private(const char *veid, int layout, const char *path)
{
	char buf[PATH_MAX], sveid[STR_SIZE], *ptr;
	...
	if (realpath(path, buf) == NULL) {
		if (errno == ENOENT) {
			...
			logger(-1, errno, "realpath function for path \"%s\" returned error", path);
			return -1;
		}
	}
